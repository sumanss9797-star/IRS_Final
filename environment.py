...
class RIS_MISO_PDA(gym.Env):
    ...
    def step(self, action, custom_betas=None):
        ...
        reward, opt_reward, individual_rates = self._compute_reward(self.Phi)

        if self.mismatch:
            true_reward = reward
            true_individual_rates = individual_rates
        else:
            true_reward, opt_reward, true_individual_rates = self._compute_reward(self.Phi_mismatch)

        done = opt_reward == reward or self.episode_t >= self._max_episode_steps

        self.info["true reward"] = true_reward
        self.info["individual_rates"] = true_individual_rates
