...
class RIS_MISO_Env(gym.Env):
    ...
    def __init__(self, 
        ...
        # NOTE UserWarning: WARN: A Box observation space has an unconventional shape (neither an image, nor a 1D vector). 
        # We recommend flattening the observation to have only a 1D vector or use a custom policy to properly process the data. 
        # Actual observation shape: (1, 336)
        self.observation_space = spaces.Box(low=-1000, high=1000, shape=(self.state_dim,), dtype=float)
        # print(self.observation_space.shape)  # shape: (336,)
        # print(self.observation_space.dtype)  # dtype: float64
    ...
    def np_ULA_response(self, angle: np.float32, num_antennas: np.uint8) -> np.ndarray:
        """
        Return the Uniform Linear Array (ULA) steering vector
    ...
    def np_UPA_response(self, azimuth: np.float32, elevation: np.float32, M_y: np.uint8, M_z: np.uint8) -> np.ndarray:
        """
        Return the Uniform Planar Array (UPA) steering vector
    ...
    def np_USPA_response(self, azimuth: np.float32, elevation: np.float32, num_antennas: np.uint8) -> np.ndarray:
        """
        ...
        for m in range(num_sqrt):
            for n in range(num_sqrt):
                array_response[m * num_sqrt + n] = np.exp(1j*np.pi*(m*np.sin(azimuth)*np.cos(elevation) + n*np.cos(elevation)))

        array_response = array_response / np.sqrt(num_antennas)
        return array_response

    def np_array_response(angle1, angle2, num, antenna_array: str):
        """
        ...
        if antenna_array == "UPA":
            ...
            for m in range(num_sqrt):
                for n in range(num_sqrt):
                    array_response[m * num_sqrt + n] = np.exp(1j*np.pi*(m*np.sin(angle1)*np.cos(angle2) + n*np.cos(angle2)))
    ...
    def reset(self, seed=None, options=None):
        ...
        actions = np.array(list(self.prev_actions), dtype=float)
        ...
        observation = np.concatenate((H_1_real, H_1_imag, H_2_real, H_2_imag, H_3_real, H_3_imag, 
                                      Phi_real, Phi_imag, actions), axis=0, dtype=float)
    ...
    def _compute_min_DownLinkRate_reward(self, Phi):
        ...
        reward, opt_reward = np.min(actual_rates), np.min(optimal_rates)
        # log(actual_rates, optimal_rates, reward, opt_reward)
        return reward, opt_reward
    ...
    def _validate_MSE(self, MSEs):
        mse_real = np.real(MSEs)
        check_negativity = np.asarray([False if i < 0 else True for i in mse_real])
        # print(np.all(check_negativity))
        
        mse_imag = np.imag(MSEs)
        check_negligibility = np.asarray([False if i > 1e-10 else True for i in mse_imag])
        # print(np.all(check_negligibility))

        # log(check_negativity, check_negligibility)
        return np.all(check_negativity) and np.all(check_negligibility)
    ...
    def step(self, action):
        ...
        # ensure the action is discrete by rounding it to the nearest integer; this might be redundant
        ...
        # valid = self._validate_MSE(user_MSEs)
        ...
        reward_offset = 230
        reward, opt_reward = reward_offset - current_max_MSE, reward_offset
        # log(reward, opt_reward)

        truncated = (self.episode_t >= self._max_episode_steps)
        # truncated = True if self.episode_t >= self._max_episode_steps else False
        done = (opt_reward == reward) or truncated
        return observation, reward, done, truncated, info
        # return valid, user_MSEs
...
def get_rand_act(Ns=16, bits=3,):
    n_actions = 2 ** bits
    rand_action = np.random.randint(low=0, high=n_actions, size=Ns)
    return rand_action

def checking_env(env):
    print(f"checking if the environment follows the Gym and SB3 interfaces\n")
    # NOTE UserWarning: WARN: Not able to test alternative render modes due to the environment not having a spec. 
...
def test_env(env, episodes=4):
    for episode in range(1, episodes + 1):
        ...
        while not done:
            ...
            print(f"   prev actions: {list(env.prev_actions)}")
            obs, reward, done, truncated, info = env.step(random_action)
            print(f"   reward:  {reward}")
            print(f"-"*44)

def validate_MSE_assupmtions(env, episodes=100):
    for episode in range(1, episodes + 1):
        print(f"{episode}")
        obs = env.reset()
        random_action = env.action_space.sample()
        valid, user_MSEs = env.step(random_action)
        if valid == False: 
            log(user_MSEs)
            break
...
def plot_MSE_dist(env, episodes=100):
    ...
    with open("log/mse_samples.txt", "w") as txt_file:
    ...
    plt.axvline(x=np.mean(mse_samples), linewidth=1.5, linestyle='dashed', color='black', label='Average MSE')
    ...
    plt.axvline(x=np.median(mse_samples), linewidth=1.5, linestyle='dashdot', color='yellow', label='Median MSE')
    plt.hist(mse_samples, bins=num_bins, label='MSE values')
    plt.legend()
