...
class PhysicalCheckpointCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        # the vectorized environment places info in a list for each env
        ...
        if self.num_timesteps % 100 == 0:
            rates = info.get('individual_rates', [])
            outages = sum(1 for r in rates if r < 1.0)
            rates_str = " | ".join([f"U{i+1}: {r:.2f}" for i, r in enumerate(rates)])
            print(f"Time step: {self.num_timesteps} Max. Min-Max Reward: {self.max_mismatch_reward:.3f} | {rates_str} | Outages: {outages}")
...
if __name__ == "__main__":
    ...
    model = PPO('MlpPolicy', env, policy_kwargs=policy_kwargs, ent_coef=0.01, verbose=0, tensorboard_log=logs_dir)

    chkpt_callback = PhysicalCheckpointCallback()

    for i in range(1, episodes + 1):
        model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name=f"{model_name}", callback=chkpt_callback)
    ...
    if chkpt_callback.best_action is not None:
        ...
        rates = info.get('individual_rates', [])
        outages = sum(1 for r in rates if r < 1.0)
        rates_str = " | ".join([f"U{j+1}: {r:.2f}" for j, r in enumerate(rates)])
        print(f"Physical Checkpoint Restored! True Min-Max Reward: {reward:.3f} | {rates_str} | Outages: {outages}")
        print("------------------------------------------------------\n")
